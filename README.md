...
3. 重启 MaiBot
4. 如果你是一键包用户又恰巧不会装依赖，那你可以参考以下步骤：打开![[点我启动!!!.bat → 输入11 → 输入1 → 将“aiohttp aiofiles Pillow httpx”复制粘贴到输入框 → 按下回车然后等待跳出来的框框里的程序跑完 → 关闭跳出来的那个框（其他插件的依赖也可以这样安装哦）
...
配置说明
1. 访问 NapCat WebUI 
2. 进入「网络配置」→「新建」→「HTTP服务器」 
3. 名称随便填一个，port填9999，host填127.0.0.1
4. （可选）更改或直接删除token（如果不为空那就需要配置完这个之后在插件配置里填一下，不嫌麻烦可以弄）
5. 点击左上角的启用，然后点击右下角的保存
